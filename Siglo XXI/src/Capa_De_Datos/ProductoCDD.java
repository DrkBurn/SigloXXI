 ...
 * @author Aspire3
 */
public class ProductoCDD {

    String mensaje = "";

    //ingresa datos a la base de datos
    public String crear(Connection conn, Producto p) {
        mensaje = "";
        PreparedStatement pst = null;
        String sql = "INSERT INTO producto (id_usuario,nombre_producto) VALUES "
                + "(?,?)";
        try {
    ...
    public Producto[] leer(Connection conn) {
        ...
        try {
            st = conn.createStatement();
            rs = st.executeQuery(sql);
            Producto data[] = new Producto[n_arreglo];
            int posicion = 0;
            while (rs.next()) {

                data[posicion] =new Producto();
                data[posicion].setId_producto(Integer.parseInt(rs.getString(1)));
                data[posicion].setNombre_producto(rs.getString(2));
    ...
    //Buscar especifico
    public Producto BuscarId(Connection conn, String id) {
        String sql;
        Statement st = null;
        ResultSet rs = null;

        sql = "Select * from producto where id_producto = '" + id + "' order by id_producto";
        try {
            st = conn.createStatement();
            rs = st.executeQuery(sql);
            Producto p =new Producto();
            while (rs.next()) {
                ...
                p.setId_producto(Integer.parseInt(rs.getString(1)));
    ...
    public String modificar(Connection conn, Producto p) {
        mensaje = "";
        PreparedStatement pst = null;
        String sql = "UPDATE producto SET stock_disponiblr = ? WHERE id_producto = ?";
        try {
            pst = conn.prepareStatement(sql);

            pst.setInt(2, p.getId_producto());
